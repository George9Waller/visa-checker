...
export function SchengenProjectionChart({
  ...
  const W = 600, H = 140, PL = 36, PR = 16, PT = 20, PB = 30;
  const plotW = W - PL - PR;
  const plotH = H - PT - PB;
  ...
  const todayIdx = points.reduce(
    (best, p, i) =>
      Math.abs(p.date.getTime() - todayMs) < Math.abs(points[best].date.getTime() - todayMs)
        ? i
        : best,
    0,
  );

  const minIdx = points.reduce((m, p, i) => (p.remaining < points[m].remaining ? i : m), 0);
  ...
  const gridLines = limit === 90 ? [0, 30, 60, 90] : [0, Math.round(limit / 3), Math.round((2 * limit) / 3), limit];
  const endDate = points[points.length - 1]?.date ?? new Date(today.getTime() + 365 * 86400000);

  return (
    <div className="p-4 rounded-lg border border-border bg-bg-raised">
      <div className="flex justify-between items-baseline mb-1">
        <div className="font-body font-semibold text-md text-fg">Days left over the next year</div>
        <div className="font-mono text-[10px] text-fg-faint tracking-wide">
      ...
      <svg viewBox={`0 0 ${W} ${H}`} style={{ width: '100%', height: 'auto', display: 'block' }}>
        ...
        <rect x={PL} y={y(15)} width={plotW} height={y(0) - y(15)} fill="var(--color-danger)" opacity="0.05" />

        {/* Area fill */}
        <path d={areaRemaining} fill="var(--color-accent)" opacity="0.12" />

        {/* Projection line */}
        <path d={pathRemaining} fill="none" stroke="var(--color-accent)" strokeWidth="1.8" />

        {/* TODAY marker */}
        <line
          x1={x(todayIdx)} y1={PT} x2={x(todayIdx)} y2={y(0)}
          stroke="var(--color-fg)" strokeWidth="0.8" strokeDasharray="2,2"
        />
        <text x={x(todayIdx) + 4} y={PT + 10} fontSize="9" fill="var(--color-fg)" fontFamily="var(--font-mono)">
          TODAY
        </text>

        {/* Min remaining point */}
        <circle
          cx={x(minIdx)} cy={y(minPoint.remaining)} r="3.5"
          fill="var(--color-bg-raised)" stroke="var(--color-danger)" strokeWidth="1.5"
        />
        <text
          x={x(minIdx)} y={y(minPoint.remaining) - 8}
        ...
        {monthLabels.map((m, i) => (
          <text
            key={i} x={x(m.i)} y={H - 10}
            fontSize="9" fill="var(--color-fg-muted)"
